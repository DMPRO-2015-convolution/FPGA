...
class Tile( img_width: Int, 
    ...
    val test = Module(new BRAMtest())

    test.io.data_in := io.hdmi_data_in
    test.io.enq := io.hdmi_input_valid
    test.io.deq := io.request_processed_data
    io.data_out := test.io.data_out
    io.output_valid := test.io.can_deq
